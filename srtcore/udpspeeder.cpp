...
const char UDPspeederFilter::defaultConfig[] = "udpspeeder,fec:20-10,mode:0";

// UDPspeeder filter will be registered when PacketFilter::internal() is called
...
// UDPspeederAntiReplay implementation
UDPspeederAntiReplay::UDPspeederAntiReplay() : index(0)
{
    clear();
}

void UDPspeederAntiReplay::clear()
{
    memset(replay_buffer, 0xFF, sizeof(replay_buffer)); // Fill with -1
    replay_map.clear();
    replay_map.rehash(UDPSPEEDER_ANTI_REPLAY_BUFF_SIZE * 3);
    index = 0;
}

bool UDPspeederAntiReplay::isValid(uint32_t seq)
{
    ...
    if (current_time - it->second.timestamp > UDPSPEEDER_ANTI_REPLAY_TIMEOUT)
    {
...
void UDPspeederAntiReplay::setInvalid(uint32_t seq)
{
    if (!isValid(seq))
    ...
    index++;
    if (index >= UDPSPEEDER_ANTI_REPLAY_BUFF_SIZE)
        index = 0;
}

// UDPspeederBlobEncoder implementation
UDPspeederBlobEncoder::UDPspeederBlobEncoder()
{
    clear();
}

void UDPspeederBlobEncoder::clear()
{
    counter = 0;
    current_len = sizeof(uint32_t); // Reserve space for packet count
}

int UDPspeederBlobEncoder::getShardLen(int n) const
{
    return round_up_div(current_len, n);
}

int UDPspeederBlobEncoder::getShardLen(int n, int next_packet_len) const
{
    return round_up_div(current_len + sizeof(uint16_t) + next_packet_len, n);
}

int UDPspeederBlobEncoder::input(const char* data, int len)
{
    ...
    counter++;
    if (counter > UDPSPEEDER_MAX_BLOB_PACKETS)
        return -1;
...
int UDPspeederBlobEncoder::output(int n, char**& data_arr, int& shard_len)
{
...
// UDPspeederBlobDecoder implementation
UDPspeederBlobDecoder::UDPspeederBlobDecoder()
{
    clear();
}

void UDPspeederBlobDecoder::clear()
{
    current_len = 0;
    last_len = -1;
    counter = 0;
...
int UDPspeederBlobDecoder::input(const char* data, int len)
{
    if (last_len != -1 && last_len != len)
        return -1; // Length mismatch
    
    counter++;
    if (counter > UDPSPEEDER_MAX_FEC_PACKETS)
        return -1;
...
int UDPspeederBlobDecoder::output(int& n, char**& data_arr, int*& len_arr)
{
    int parser_pos = 0;
    ...
    if (n > UDPSPEEDER_MAX_BLOB_PACKETS)
        return -1;
...
// UDPspeederFilter implementation
UDPspeederFilter::UDPspeederFilter(const SrtFilterInitializer& init, std::vector<SrtPacket>& provided, const std::string& confstr)
...
UDPspeederFilter::~UDPspeederFilter()
{
    // Cleanup is automatic with RAII
}

bool UDPspeederFilter::verifyConfig(const SrtFilterConfig& config, std::string& w_errormsg)
{
    // Basic validation - more detailed validation in parseConfig
    if (config.type != "udpspeeder")
    {
        w_errormsg = "Filter type must be 'udpspeeder'";
        return false;
    }
...
uint64_t UDPspeederFilter::getCurrentTime() const
{
    return std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::steady_clock::now().time_since_epoch()).count();
}

bool UDPspeederFilter::parseConfig(const std::string& confstr)
{
    SrtFilterConfig cfg;
    ...
    for (const auto& param : cfg.parameters)
    {
        if (param.first == "fec")
        {
            // Parse FEC parameter in format "x-y" or "x1-y1,x2-y2,..."
            // Changed from colon to dash to avoid conflict with SRT config parser
            std::vector<std::string> fec_parts;
            std::stringstream ss(param.second);
            std::string item;

            while (std::getline(ss, item, ','))
            {
                fec_parts.push_back(item);
            }

            for (const auto& fec_part : fec_parts)
            {
                size_t dash_pos = fec_part.find('-');
                if (dash_pos == std::string::npos)
                    return false;

                int x = std::atoi(fec_part.substr(0, dash_pos).c_str());
                int y = std::atoi(fec_part.substr(dash_pos + 1).c_str());

                if (x < 1 || y < 0 || x + y > UDPSPEEDER_MAX_FEC_PACKETS)
                    return false;

                UDPspeederFECParam fec_param;
                fec_param.x = static_cast<uint8_t>(x);
                fec_param.y = static_cast<uint8_t>(y);
    ...
    {
        UDPspeederFECParam default_param;
        default_param.x = 20;
        default_param.y = 10;
...
UDPspeederFECParam UDPspeederFilter::getFECParam(int packet_count) const
{
    ...
    UDPspeederFECParam result = m_config.fec_params[0];
...
void UDPspeederFilter::writeHeader(char* buf, const UDPspeederHeader& header)
{
    int offset = 0;
...
bool UDPspeederFilter::readHeader(const char* buf, int len, UDPspeederHeader& header)
{
    if (len < static_cast<int>(sizeof(UDPspeederHeader)))
        return false;
...
bool UDPspeederFilter::packControlPacket(SrtPacket& packet, int32_t seq)
{
...
void UDPspeederFilter::feedSource(CPacket& packet)
{
...
int UDPspeederFilter::encodePackets()
{
    if (m_blob_encoder.getPacketCount() == 0)
        return -1;

    UDPspeederFECParam fec_param = getFECParam(m_blob_encoder.getPacketCount());
    ...
    {
        SrtPacket pkt(shard_len + sizeof(UDPspeederHeader));

        UDPspeederHeader header;
        ...
        memcpy(pkt.data() + sizeof(UDPspeederHeader), rs_shards[i], shard_len);
...
bool UDPspeederFilter::receive(const CPacket& pkt, loss_seqs_t& loss_seqs)
{
    ...
    // Extract payload
    const char* payload = data + sizeof(UDPspeederHeader);
    int payload_len = len - sizeof(UDPspeederHeader);
...
bool UDPspeederFilter::receiveBlobMode(const UDPspeederHeader& header, const char* payload, int payload_len, loss_seqs_t& loss_seqs)
{
...
bool UDPspeederFilter::receivePacketMode(const UDPspeederHeader& header, const char* payload, int payload_len, loss_seqs_t& loss_seqs)
{
    (void)loss_seqs; // Suppress unused parameter warning
...
int UDPspeederFilter::decodePackets(uint32_t seq)
{
    auto group_it = m_fec_groups.find(seq);
    if (group_it == m_fec_groups.end())
        return -1;
